...
class FirestoreService {
  ...
  Future<List<User>> searchUsers(String query) async {
    ...
    try {
      ...
      final lowerQuery = query.toLowerCase().trim(); // Chuyển từ khóa tìm kiếm về chữ thường
  ...
  Future<void> sendFriendRequest(String senderId, String senderName, String receiverId) async {
    // Kiểm tra xem đã gửi chưa để tránh spam
    final existing = await _db.collection('friend_requests')
  ...
  Future<List<User>> getFriends(String currentUserId) async {
    try {
      ...
      final sentQuery = await _db.collection('friend_requests')
      ...
      final receivedQuery = await _db.collection('friend_requests')
          .where('receiverId', isEqualTo: currentUserId)
          .where('status', isEqualTo: 'accepted')
          .get();
